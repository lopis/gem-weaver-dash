...
1. Collapse `hash` + `pathNoise` into one helper
...
1. Reuse one `performance.now()` per draw/update tick
...
## 2026-09-09 - Deduplicate repeated level payloads

### Goal
Reduce literal payload by reusing one canonical level string instead of repeating the same encoded map data across several level entries.

### Baseline

- Baseline before this test: 12979 B (`dist/index.zip`)

### Variant tested

1. Reused a single `level8` constant for levels 8 through 13

- The same encoded payload was repeated across several entries in [src/game/level-data.ts](src/game/level-data.ts).
- Result: 12953 B
- Delta vs baseline: -26 B (improvement)
